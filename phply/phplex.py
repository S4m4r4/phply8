...
# Asymmetric visibility (PHP 8.4) - must come before STRING to match first
def t_php_ASYMMETRIC_VISIBILITY(t):
    r"(private|public|protected)\s*\(\s*(set|get)\s*\)"
    visibility = t.value.split("(")[0].strip().upper()
    t.type = visibility + "_SET"  # For now, treat all as _SET
    return t
...
# Floating literal
def t_php_DNUMBER(t):
    r"(\d[0-9_]*\.[0-9_]*|\.\d[0-9_]*)([Ee][+-]?\d[0-9_]*)?|\d[0-9_]*[Ee][+-]?\d[0-9_]*"
...
# Integer literal
def t_php_LNUMBER(t):
    r"(0b[01_]+)|(0x[0-9A-Fa-f_]+)|\d[0-9_]*"
    t.value = t.value.replace("_", "")  # Remove underscores for parsing
    return t
