...
def p_attribute_group(p):
    "attribute_group : ATTR_START attribute_list RBRACKET"
    p[0] = ast.AttributeGroup(p[2], lineno=p.lineno(1))


def p_attribute_groups(p):
    """attribute_groups : attribute_groups attribute_group
    | attribute_group"""
    if len(p) == 3:
        if isinstance(p[1], ast.AttributeGroup):
            p[0] = ast.AttributeGroup(p[1].attrs + p[2].attrs, lineno=p[1].lineno)
        else:
            p[0] = p[2]
    else:
        p[0] = p[1]
...
def p_attribute(p):
    """attribute : class_name_reference
    | class_name_reference LPAREN function_call_parameter_list RPAREN
    | class_name_reference LPAREN RPAREN"""
    if len(p) == 2:
        p[0] = ast.Attribute(p[1], [], lineno=p.lineno(1))
    elif len(p) == 4:
        p[0] = ast.Attribute(p[1], [], lineno=p.lineno(1))
    else:
        p[0] = ast.Attribute(p[1], p[3], lineno=p.lineno(1))
...
def p_top_statement_use(p):
    ...
    if len(p) == 4:
        ...
        p[0] = ast.UseDeclarations(p[3], lineno=p.lineno(1))
...
def p_inner_statement_yield(p):
    ...
    if len(p) == 3:
        ...
        p[0] = ast.Yield(p[2], None, lineno=p.lineno(1))
    else:
        p[0] = ast.Yield(p[4], p[2], lineno=p.lineno(1))
...
def p_statement_if(p):
    ...
    if len(p) == 8:
        ...
        p[0] = ast.If(
            p[3], ast.Block(p[6], lineno=p.lineno(5)), p[7], p[8], lineno=p.lineno(1)
        )
...
def p_additional_catches(p):
    """additional_catches : additional_catches CATCH LPAREN type_hint VARIABLE RPAREN LBRACE inner_statement_list RBRACE
    | additional_catches CATCH LPAREN type_hint RPAREN LBRACE inner_statement_list RBRACE
    | empty"""
    if len(p) == 10:
        p[0] = p[1] + [
            ast.Catch(
                p[4], ast.Variable(p[5], lineno=p.lineno(5)), p[8], lineno=p.lineno(2)
            )
        ]
    elif len(p) == 9:
        # PHP 8.0
...
def p_new_elseif_list(p):
    ...
    if len(p) == 2:
        ...
        p[0] = p[1] + [
            ast.ElseIf(p[4], ast.Block(p[7], lineo=p.lineno(6)), lineno=p.lineno(2))
        ]
...
def p_foreach_variable(p):
    ...
    if len(p) == 5:
        ...
        p[0] = ast.ForeachVariable(p[2], False, lineno=p.lineno(1))
    elif len(p) == 2:
...
def p_identifier(p):
    ...
    if isinstance(p[1], str):
        p[0] = p[1].lower() if p[1] in keywords else p[1]
    else:
        p[0] = p[1]


def p_function_declaration_statement(p):
    """function_declaration_statement : FUNCTION is_reference identifier LPAREN parameter_list RPAREN return_type LBRACE inner_statement_list RBRACE
    | attribute_groups FUNCTION is_reference identifier LPAREN parameter_list RPAREN return_type LBRACE inner_statement_list RBRACE"""
    if len(p) == 11:
        p[0] = ast.Function(p[3], p[5], p[9], p[2], p[7], lineno=p.lineno(1))
    else:
        p[0] = ast.Function(p[4], p[6], p[10], p[3], p[8], lineno=p.lineno(2))
...
def p_class_declaration_statement(p):
    ...
    | attribute_groups class_entry_type STRING extends_from implements_list LBRACE class_statement_list RBRACE
    ...
    | ENUM STRING enum_type implements_list LBRACE class_statement_list RBRACE
    | attribute_groups ENUM STRING enum_type implements_list LBRACE class_statement_list RBRACE"""

    has_attr = len(p) > 2 and isinstance(p[1], ast.AttributeGroup)
    offset = 1 if has_attr else 0

    is_enum = False
    if has_attr and getattr(p[2], "lower", None):
        is_enum = p[2].lower() == "enum"
    elif not has_attr and getattr(p[1], "lower", None):
        is_enum = p[1].lower() == "enum"
    ...
    if is_enum:
        p[0] = ast.Enum(
            ...
            p[4 + offset],
            p[6 + offset],
            lineno=p.lineno(1 + offset),
        )
    elif (len(p) == 8 and not has_attr) or (len(p) == 9 and has_attr):
        traits = []
        stmts = []
        for s in p[6 + offset]:
...
def p_trait_name_list(p):
    """trait_name_list : trait_name_list COMMA fully_qualified_class_name
...
def p_trait_member(p):
    """trait_member : fully_qualified_class_name DOUBLE_COLON identifier
    | identifier"""
    if len(p) == 4:
...
def p_trait_modifier(p):
    """trait_modifier : trait_member AS PUBLIC identifier SEMI
    | trait_member AS PROTECTED identifier SEMI
    | trait_member AS PRIVATE identifier SEMI
    | trait_member AS PUBLIC SEMI
    | trait_member AS PROTECTED SEMI
    | trait_member AS PRIVATE SEMI
    | trait_member AS identifier SEMI"""
    if len(p) == 6:
        p[0] = ast.TraitModifier(p[1], p[4], p[3].lower())
    elif len(p) == 5 and p[3] in ("public", "protected", "private"):
        p[0] = ast.TraitModifier(p[1], None, p[3].lower())
    else:
...
def p_trait_statement(p):
    """trait_statement : method_modifiers FUNCTION is_reference identifier LPAREN parameter_list RPAREN return_type method_body
    | variable_modifiers class_variable_declaration SEMI
    | attribute_groups variable_modifiers class_variable_declaration SEMI
    | USE trait_name_list LBRACE trait_modifiers_list RBRACE
    | USE trait_name_list SEMI"""
    has_attr = len(p) > 3 and isinstance(p[1], ast.AttributeGroup)
    offset = 1 if has_attr else 0

    if len(p) == 10:
        p[0] = ast.Method(p[4], p[1], p[6], p[9], p[3], p[8], lineno=p.lineno(2))
    elif len(p) == 6:
        p[0] = ast.TraitUse(p[2], p[4], lineno=p.lineno(1))
    elif (len(p) == 5 and has_attr) or (len(p) == 4 and not has_attr):
        if p[1 + offset] == "use":
            p[0] = ast.TraitUse(p[2 + offset], [], lineno=p.lineno(1))
        else:
            p[0] = ast.ClassVariables(
                p[1 + offset],
                p[2 + offset],
                getattr(p[2 + offset][0], "type", None) if p[2 + offset] else None,
                lineno=p.lineno(3 + offset),
            )
...
def p_type_hint(p):
    """type_hint : type_hint OR type_hint_item
    | type_hint AND type_hint_item
    | LPAREN type_hint OR type_hint_item RPAREN
    | LPAREN type_hint AND type_hint_item RPAREN
    | type_hint_item"""
    if len(p) == 4:
        p[0] = p[1] + p[2] + p[3]
    elif len(p) == 6:
        # Parenthesized complex type (DNF)
        p[0] = "(" + p[2] + p[3] + p[4] + ")"
    else:
        p[0] = p[1]


def p_type_hint_item(p):
    """type_hint_item : class_name
    | ARRAY
    | QUESTION class_name
    | QUESTION ARRAY"""
    if len(p) == 2:
...
def p_class_statement(p):
    """class_statement : method_modifiers FUNCTION is_reference identifier LPAREN parameter_list RPAREN return_type method_body
    | attribute_groups method_modifiers FUNCTION is_reference identifier LPAREN parameter_list RPAREN return_type method_body
    | variable_modifiers class_variable_declaration SEMI
    | variable_modifiers class_variable_declaration
    | attribute_groups variable_modifiers class_variable_declaration SEMI
    | attribute_groups variable_modifiers class_variable_declaration
    | class_constant_declaration SEMI
    | USE trait_name_list LBRACE trait_modifiers_list RBRACE
    | USE trait_name_list SEMI
    | CASE identifier EQUALS static_scalar SEMI
    | CASE identifier SEMI"""

    has_attr = len(p) > 2 and isinstance(p[1], ast.AttributeGroup)
    offset = 1 if has_attr else 0

    if (len(p) == 10 and not has_attr) or (len(p) == 11 and has_attr):
        p[0] = ast.Method(
            p[4 + offset],
            p[1 + offset],
            p[6 + offset],
            ...
            p[3 + offset],
            p[8 + offset],
            lineno=p.lineno(2 + offset),
        )
    elif (
        len(p) == 4
        and not has_attr
        and getattr(p[2], "__class__", None)
        and p[2].__class__.__name__ == "ClassVariable"
    ) or (
        len(p) == 5
        and has_attr
        and getattr(p[3], "__class__", None)
        and p[3].__class__.__name__ == "ClassVariable"
    ):
        lineno_idx = 3 + offset if len(p) > 3 else 2 + offset
        p[0] = ast.ClassVariables(
            p[1 + offset],
            p[2 + offset],
            getattr(p[2 + offset][0], "type", None) if p[2 + offset] else None,
            lineno=p.lineno(lineno_idx) if lineno_idx < len(p) else p.lineno(1),
        )
    elif (
        len(p) == 3
        and not has_attr
    ...
    elif len(p) == 6 and isinstance(p[1], str) and p[1].lower() == "case":
        p[0] = ast.EnumCase(p[2], p[4], lineno=p.lineno(1))
    elif len(p) == 6:
        p[0] = ast.TraitUse(p[2], p[4], lineno=p.lineno(1))
    elif len(p) == 4 and isinstance(p[1], str) and p[1].lower() == "case":
        ...
        if p[1] == "use":
            ...
            p[0] = ast.ClassVariables(
                p[1],
                p[2],
                getattr(p[2][0], "type", None) if p[2] else None,
                lineno=p.lineno(3),
            )
    else:
...
def p_class_variable_declaration_initial(p):
    """class_variable_declaration : class_variable_declaration COMMA VARIABLE EQUALS static_expr
    | type_hint VARIABLE EQUALS static_expr
    | VARIABLE EQUALS static_expr"""
    if len(p) == 6:
        p[0] = p[1] + [ast.ClassVariable(p[3], p[5], lineno=p.lineno(2))]
    elif len(p) == 5:
        cv = ast.ClassVariable(p[2], p[4], lineno=p.lineno(1))
        cv.type = p[1]
        p[0] = [cv]
    else:
...
def p_class_variable_declaration_no_initial(p):
    ...
    | type_hint VARIABLE property_hooks
    | type_hint VARIABLE
    | VARIABLE"""
    if len(p) == 4 and p[2] == ",":
        p[0] = p[1] + [ast.ClassVariable(p[3], None, lineno=p.lineno(2))]
    elif len(p) == 4:
        cv = ast.ClassVariable(p[2], None, lineno=p.lineno(1))
        cv.type = p[1]
        p[0] = [cv]
    elif len(p) == 3:
        cv = ast.ClassVariable(p[2], None, lineno=p.lineno(1))
...
def p_property_hooks(p):
    """property_hooks : LBRACE property_hook_list RBRACE"""
    pass
...
def p_property_hook(p):
    """property_hook : identifier SEMI
    | identifier DOUBLE_ARROW expr SEMI
    | identifier LBRACE inner_statement_list RBRACE"""
    pass


def p_class_constant_declaration(p):
    """class_constant_declaration : class_constant_declaration COMMA identifier EQUALS static_expr
    | variable_modifiers CONST type_hint identifier EQUALS static_expr
    | variable_modifiers CONST identifier EQUALS static_expr
    | CONST type_hint identifier EQUALS static_expr
    | CONST identifier EQUALS static_expr"""
    if len(p) == 7 and p[2].lower() == "const":
        p[0] = [ast.ClassConstant(p[4], p[6], p[1], lineno=p.lineno(2))]
    elif len(p) == 6 and isinstance(p[1], list):
        p[0] = p[1] + [
            ast.ClassConstant(
                p[3],
                p[5],
                p[1][0].modifiers if p[1] and hasattr(p[1][0], "modifiers") else [],
                lineno=p.lineno(2),
            )
        ]
    elif len(p) == 6 and isinstance(p[2], str) and p[2].lower() == "const":
        p[0] = [ast.ClassConstant(p[3], p[5], p[1], lineno=p.lineno(2))]
    elif len(p) == 6 and isinstance(p[1], str) and p[1].lower() == "const":
        p[0] = [ast.ClassConstant(p[3], p[5], [], lineno=p.lineno(1))]
    else:
...
def p_member_modifier_visibility(p):
    ...
    | PROTECTED_SET
    | PRIVATE_SET"""
    if p[1] in ("public_set", "protected_set", "private_set"):
        p[0] = p[1].replace("_set", "")  # Extract base visibility
    else:
        p[0] = p[1].lower()
...
def p_parameter(p):
    ...
    | attribute_groups parameter_item"""
    if len(p) == 3:
        p[0] = p[2]
    else:
        p[0] = p[1]
...
def p_parameter_item(p):
    """parameter_item : VARIABLE
    | type_hint VARIABLE
    | READONLY type_hint VARIABLE
    | PUBLIC type_hint VARIABLE
    | PROTECTED type_hint VARIABLE
    | PRIVATE type_hint VARIABLE
    | PUBLIC_SET type_hint VARIABLE
    | PROTECTED_SET type_hint VARIABLE
    | PRIVATE_SET type_hint VARIABLE
    | PUBLIC READONLY type_hint VARIABLE
    | PROTECTED READONLY type_hint VARIABLE
    | PRIVATE READONLY type_hint VARIABLE
    | PUBLIC_SET READONLY type_hint VARIABLE
    | PROTECTED_SET READONLY type_hint VARIABLE
    | PRIVATE_SET READONLY type_hint VARIABLE
    | AND VARIABLE
    | type_hint AND VARIABLE
    | PUBLIC type_hint AND VARIABLE
    | PROTECTED type_hint AND VARIABLE
    | PRIVATE type_hint AND VARIABLE
    | PUBLIC READONLY type_hint AND VARIABLE
    | PROTECTED READONLY type_hint AND VARIABLE
    | PRIVATE READONLY type_hint AND VARIABLE
    | VARIABLE EQUALS static_expr
    | type_hint VARIABLE EQUALS static_expr
    | READONLY type_hint VARIABLE EQUALS static_expr
    | PUBLIC type_hint VARIABLE EQUALS static_expr
    | PROTECTED type_hint VARIABLE EQUALS static_expr
    | PRIVATE type_hint VARIABLE EQUALS static_expr
    | PUBLIC_SET type_hint VARIABLE EQUALS static_expr
    | PROTECTED_SET type_hint VARIABLE EQUALS static_expr
    | PRIVATE_SET type_hint VARIABLE EQUALS static_expr
    | PUBLIC READONLY type_hint VARIABLE EQUALS static_expr
    | PROTECTED READONLY type_hint VARIABLE EQUALS static_expr
    | PRIVATE READONLY type_hint VARIABLE EQUALS static_expr
    | PUBLIC_SET READONLY type_hint VARIABLE EQUALS static_expr
    | PROTECTED_SET READONLY type_hint VARIABLE EQUALS static_expr
    | PRIVATE_SET READONLY type_hint VARIABLE EQUALS static_expr
    | AND VARIABLE EQUALS static_expr
    | type_hint AND VARIABLE EQUALS static_expr
    | PUBLIC type_hint AND VARIABLE EQUALS static_expr
    | PROTECTED type_hint AND VARIABLE EQUALS static_expr
    | PRIVATE type_hint AND VARIABLE EQUALS static_expr
    | PUBLIC READONLY type_hint AND VARIABLE EQUALS static_expr
    | PROTECTED READONLY type_hint AND VARIABLE EQUALS static_expr
    | PRIVATE READONLY type_hint AND VARIABLE EQUALS static_expr
    | ELLIPSIS VARIABLE
    | type_hint ELLIPSIS VARIABLE
    | AND ELLIPSIS VARIABLE
    | type_hint AND ELLIPSIS VARIABLE"""
    ...
    if parts and parts[0] == "readonly":
        is_readonly = True
        parts.pop(0)
    elif parts and parts[0] in ("public_set", "protected_set", "private_set"):
        modifier = parts.pop(0).replace("_set", "")
    elif parts and parts[0] in ("public", "protected", "private"):
        modifier = parts.pop(0).lower()
        if parts and parts[0] == "readonly":
            is_readonly = True
            parts.pop(0)
    ...
    if len(parts) == 1:
        ...
        if parts[1] == "=":
            # Just variable and equals (default comes from next token)
            name = parts[0]
        else:
            type_ = parts[0]
            name = parts[1]
    elif len(parts) == 3:
        # Either: variable = default, or: type variable (with trailing token)
        if parts[1] == "=":
            name = parts[0]
            default = parts[2]
        else:
    ...
    p[0] = ast.FormalParameter(
        name, default, is_ref, type_, modifier, is_variadic, lineno=p.lineno(1)
    )
...
def p_expr_new(p):
    ...
    | NEW CLASS ctor_arguments extends_from implements_list LBRACE class_statement_list RBRACE
    | NEW READONLY CLASS ctor_arguments extends_from implements_list LBRACE class_statement_list RBRACE"""
    if len(p) == 4:
        ...
        for s in p[7]:
            if isinstance(s, ast.TraitUse):
                ...
                stmts.append(s)
        p[0] = ast.AnonymousClass(p[3], p[4], p[5], traits, stmts, lineno=p.lineno(1))
    else:
        traits = []
        stmts = []
        for s in p[8]:
            if isinstance(s, ast.TraitUse):
                ...
                stmts.append(s)
        p[0] = ast.AnonymousClass(p[4], p[5], p[6], traits, stmts, lineno=p.lineno(1))


def p_expr_match(p):
    "expr : MATCH LPAREN expr RPAREN LBRACE match_arm_list RBRACE"
...
def p_match_arm(p):
    """match_arm : expr_list DOUBLE_ARROW expr
    | DEFAULT DOUBLE_ARROW expr"""
    if p[1] == "default":
        p[0] = ast.MatchArm(None, p[3], lineno=p.lineno(1))
    else:
...
def p_expr_objectop(p):
    ...
    | expr NULLSAFE_OBJECT_OPERATOR object_property method_or_not"""
    name, _dims = p[3]
    assert _dims == []
    params = p[4]
    ...
    if params is not None:
        ...
        if is_nullsafe:
            p[0] = ast.NullsafeObjectProperty(p[1], name, lineno=p.lineno(3))
        else:
...
def p_expr_list_assign(p):
    ...
    | LBRACKET array_pair_list RBRACKET EQUALS expr"""
    if p[1].lower() == "list":
        p[0] = ast.ListAssignment(p[3], p[6], lineno=p.lineno(1))
    else:
        p[0] = ast.ListAssignment(p[2], p[5], lineno=p.lineno(1))
...
def p_variable(p):
    ...
    | base_variable_with_function_calls NULLSAFE_OBJECT_OPERATOR object_property method_or_not variable_properties
    ...
    if len(p) == 6:
        ...
        if params is not None:
            if is_nullsafe:
                p[0] = ast.NullsafeMethodCall(p[1], name, params, lineno=p.lineno(2))
            else:
                p[0] = ast.MethodCall(p[1], name, params, lineno=p.lineno(2))
        else:
            if is_nullsafe:
                p[0] = ast.NullsafeObjectProperty(p[1], name, lineno=p.lineno(2))
            else:
                p[0] = ast.ObjectProperty(p[1], name, lineno=p.lineno(2))
        for class_, dim, lineno in dims:
            p[0] = class_(p[0], dim, lineno=lineno)
        for (name, dims), params, is_nullsafe_prop in p[5]:
            if params is not None:
                if is_nullsafe_prop:
                    p[0] = ast.NullsafeMethodCall(
                        p[0], name, params, lineno=p.lineno(2)
                    )
                else:
                    p[0] = ast.MethodCall(p[0], name, params, lineno=p.lineno(2))
            else:
                if is_nullsafe_prop:
                    p[0] = ast.NullsafeObjectProperty(p[0], name, lineno=p.lineno(2))
                else:
                    p[0] = ast.ObjectProperty(p[0], name, lineno=p.lineno(2))
            for class_, dim, lineno in dims:
                p[0] = class_(p[0], dim, lineno=lineno)
...
def p_function_call_static(p):
    """function_call : class_name DOUBLE_COLON identifier LPAREN function_call_parameter_list RPAREN
    | class_name DOUBLE_COLON variable_without_objects LPAREN function_call_parameter_list RPAREN
    | variable_class_name DOUBLE_COLON identifier LPAREN function_call_parameter_list RPAREN
...
def p_function_call_variable(p):
    ...
    | LPAREN expr RPAREN LPAREN function_call_parameter_list RPAREN"""
    if len(p) == 5:
        p[0] = ast.FunctionCall(p[1], p[3], lineno=p.lineno(2))
    else:
        p[0] = ast.FunctionCall(p[2], p[5], lineno=p.lineno(4))


def p_function_call_backtick_shell_exec(p):
    "function_call : BACKTICK encaps_list BACKTICK"
    p[0] = ast.FunctionCall(
        "shell_exec", [ast.Parameter(p[2], False)], lineno=p.lineno(1)
    )
...
def p_variable_property(p):
    ...
    | NULLSAFE_OBJECT_OPERATOR object_property method_or_not"""
    p[0] = (p[2], p[3], p[1] == "?->")
...
def p_expr_string_offset(p):
    ...
    | expr LBRACKET dim_offset RBRACKET"""
    if p[2] == "{":
        p[0] = ast.StringOffset(p[1], p[3], lineno=p.lineno(2))
    else:
        p[0] = ast.ArrayOffset(p[1], p[3], lineno=p.lineno(2))
...
def p_non_empty_array_pair_list_item(p):
    ...
    | non_empty_array_pair_list COMMA ELLIPSIS expr
    | AND variable
    | expr
    | ELLIPSIS expr"""
    if len(p) == 5 and p[2] == ",":
...
def p_function_call_parameter_list(p):
    """function_call_parameter_list : non_empty_function_call_parameter_list possible_comma
    | empty"""
    if len(p) == 3:
        p[0] = p[1]
    else:
...
def p_non_empty_function_call_parameter_list(p):
    """non_empty_function_call_parameter_list : non_empty_function_call_parameter_list COMMA function_call_parameter
...
def p_function_call_parameter(p):
    ...
    if len(p) == 2:
        if p[1] == "...":
            p[0] = ast.Unpack(None, lineno=p.lineno(1))  # First-class callable
        else:
            p[0] = ast.Parameter(p[1], False, None, lineno=p.lineno(1))
    elif len(p) == 3:
        if p[1] == "...":
            p[0] = ast.Unpack(p[2], lineno=p.lineno(1))  # Argument unpacking
        else:
            p[0] = ast.Parameter(p[2], True, None, lineno=p.lineno(1))
    else:
        p[0] = ast.Parameter(p[3], False, p[1], lineno=p.lineno(1))


def p_expr_function(p):
    """expr : FUNCTION is_reference LPAREN parameter_list RPAREN lexical_vars return_type LBRACE inner_statement_list RBRACE
    | STATIC FUNCTION is_reference LPAREN parameter_list RPAREN lexical_vars return_type LBRACE inner_statement_list RBRACE"""
    if len(p) == 11:
        p[0] = ast.Closure(p[4], p[6], p[9], p[2], p[7], False, lineno=p.lineno(1))
    else:
        p[0] = ast.Closure(p[5], p[7], p[10], p[3], p[8], True, lineno=p.lineno(1))


def p_expr_arrow_function(p):
    """expr : FN is_reference LPAREN parameter_list RPAREN return_type DOUBLE_ARROW expr
    | STATIC FN is_reference LPAREN parameter_list RPAREN return_type DOUBLE_ARROW expr"""
    if len(p) == 9:
        p[0] = ast.ArrowFunction(p[4], p[8], p[2], p[6], lineno=p.lineno(1))
    else:
        p[0] = ast.ArrowFunction(p[5], p[9], p[3], p[7], lineno=p.lineno(2))
...
def p_isset_variables(p):
    """isset_variables : isset_variables COMMA expr
    | expr"""
    if len(p) == 4:
        p[0] = p[1] + [p[3]]
    else:
...
def p_class_constant(p):
    """class_constant : class_name DOUBLE_COLON identifier
    | variable_class_name DOUBLE_COLON identifier"""
    if p[3] == "class":
        p[0] = p[1]
    else:
        p[0] = ast.StaticProperty(p[1], p[3], lineno=p.lineno(2))
...
def p_static_non_empty_array_pair_list_item(p):
    ...
    | static_non_empty_array_pair_list COMMA ELLIPSIS static_expr
    | static_expr
    | ELLIPSIS static_expr"""
    if len(p) == 5:
        p[0] = p[1] + [ast.Unpack(p[4], lineno=p.lineno(3))]
    elif len(p) == 4:
        p[0] = p[1] + [ast.ArrayElement(None, p[3], False, lineno=p.lineno(2))]
...
def p_static_expr(p):
    ...
    | NEW class_name LPAREN function_call_parameter_list RPAREN
    | NEW class_name
    ...
    if len(p) == 2:
        ...
        p[0] = ast.New(p[2], [], lineno=p.lineno(1))
    elif len(p) == 6:
        p[0] = ast.New(p[2], p[4], lineno=p.lineno(1))
    else:
        p[0] = ast.BinaryOp(p[2].lower(), p[1], p[3], lineno=p.lineno(2))
...
def p_namespace_name(p):
    ...
    | namespace_name NS_SEPARATOR STATIC
    | namespace_name NS_SEPARATOR READONLY
    | namespace_name NS_SEPARATOR NAMESPACE
    | STRING
    | ARRAY
    | STATIC
    | READONLY
    ...
    if len(p) == 4:
        if p[3] == "static":
            ...
            p[0] = p[1] + p[2] + "ReadOnly"
        elif p[3] == "namespace":
            p[0] = p[1] + p[2] + "Namespace"
        else:
            p[0] = p[1] + p[2] + p[3]
    elif p[1] == "static":
        p[0] = "Static"
    ...
    if p[1] == "":
        ...
        if isinstance(p[1], string_type):
            ...
            p[0] = ast.BinaryOp(
                ".",
                p[1].left,
                p[1].right + process_php_string_escapes(p[2]),
                lineno=p[1].lineno,
            )
        else:
            # worst case - insert a binaryop
            p[0] = ast.BinaryOp(
                ".", p[1], process_php_string_escapes(p[2]), lineno=p.lineno(2)
            )
...
def p_encaps_var_array_offset(p):
    ...
    p[0] = ast.ArrayOffset(
        ast.Variable(p[1], lineno=p.lineno(1)), p[3], lineno=p.lineno(2)
    )


def p_encaps_var_object_property(p):
    """encaps_var : VARIABLE OBJECT_OPERATOR STRING
    | VARIABLE NULLSAFE_OBJECT_OPERATOR STRING"""
    if p[2] == "?->":
        p[0] = ast.NullsafeObjectProperty(
            ast.Variable(p[1], lineno=p.lineno(1)), p[3], lineno=p.lineno(2)
        )
    else:
        p[0] = ast.ObjectProperty(
            ast.Variable(p[1], lineno=p.lineno(1)), p[3], lineno=p.lineno(2)
        )
...
def p_encaps_var_dollar_curly_array_offset(p):
    ...
    p[0] = ast.ArrayOffset(
        ast.Variable("$" + p[2], lineno=p.lineno(2)), p[4], lineno=p.lineno(3)
    )
...
def make_parser(debug=False):
    return yacc.yacc(debug=debug, write_tables=True)
