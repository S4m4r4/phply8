...
def to_stmt(pynode):
    if not isinstance(pynode, py.stmt):
        pynode = py.Expr(pynode, lineno=pynode.lineno, col_offset=pynode.col_offset)
    return pynode
...
def from_phpast(node):
    ...
    if isinstance(node, php.Array):
        if node.nodes:
            if node.nodes[0].key is not None:
                ...
                return py.List(
                    [from_phpast(x.value) for x in node.nodes],
                    py.Load(**pos(node)),
                    **pos(node),
                )
        else:
    ...
    if isinstance(node, php.InlineHTML):
        ...
        return py.Call(
            py.Name("inline_html", py.Load(**pos(node)), **pos(node)),
            args,
            [],
            None,
            None,
    ...
    if isinstance(node, php.Echo):
        return py.Call(
            py.Name("echo", py.Load(**pos(node)), **pos(node)),
    ...
    if isinstance(node, php.Exit):
        ...
        return py.Raise(
            py.Call(
                py.Name("Exit", py.Load(**pos(node)), **pos(node)),
                args,
                [],
                None,
                None,
    ...
    if isinstance(node, php.IsSet) and len(node.nodes) == 1:
        ...
        if isinstance(node.nodes[0], php.ObjectProperty):
            return py.Call(
                ...
                [from_phpast(node.nodes[0].node), from_phpast(node.nodes[0].name)],
                [],
                None,
                None,
                **pos(node),
    ...
    if isinstance(node, php.Empty):
        return from_phpast(
            php.UnaryOp(
                ...
                php.BinaryOp(
                    "&&",
                    php.IsSet([node.expr], lineno=node.lineno),
                    node.expr,
                    lineno=node.lineno,
                ),
                lineno=node.lineno,
    ...
    if isinstance(node, php.ListAssignment):
        return py.Assign(
            [
                py.Tuple(
                    list(map(store, list(map(from_phpast, node.nodes)))),
                    py.Store(**pos(node)),
                    **pos(node),
                )
            ],
    ...
    if isinstance(node, (php.PreIncDecOp, php.PostIncDecOp)):
        return from_phpast(
            php.Assignment(
                node.expr,
                php.BinaryOp(node.op[0], node.expr, 1, lineno=node.lineno),
                False,
                lineno=node.lineno,
            )
        )
    ...
    if isinstance(node, php.ObjectProperty):
        ...
        return py.Attribute(
            from_phpast(node.node), node.name, py.Load(**pos(node)), **pos(node)
        )
    ...
    if isinstance(node, php.Include):
        once = py.Name(
            "True" if node.once else "False", py.Load(**pos(node)), **pos(node)
        )
        return py.Call(
            py.Name("include", py.Load(**pos(node)), **pos(node)),
    ...
    if isinstance(node, php.Require):
        ...
        return py.Call(
            py.Name("require", py.Load(**pos(node)), **pos(node)),
            [from_phpast(node.expr), once],
            [],
            None,
            None,
    ...
    if isinstance(node, php.Cast):
        return py.Call(
            py.Name(casts.get(node.type, node.type), py.Load(**pos(node)), **pos(node)),
            [from_phpast(node.expr)],
            [],
            None,
            None,
            ...
            php.Block(
                ...
                + [
                    php.While(
                        ...
                        php.Block(
                            deblock(node.node) + (node.count or []), lineno=node.lineno
                        ),
                        lineno=node.lineno,
                    )
                ],
    ...
    if isinstance(node, php.Foreach):
        if node.keyvar is None:
            target = py.Name(node.valvar.name[1:], py.Store(**pos(node)), **pos(node))
        else:
            target = py.Tuple(
                [
    ...
    if isinstance(node, php.Function):
        ...
        return py.FunctionDef(
            node.name, py.arguments(args, None, None, defaults), body, [], **pos(node)
        )
    ...
    if isinstance(node, (php.ClassConstants, php.ClassVariables)):
        ...
        return py.Assign([store(from_phpast(name))], from_phpast(initial), **pos(node))
    ...
    return py.Call(
        py.Name("XXX", py.Load(**pos(node)), **pos(node)),
